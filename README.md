...
<table>
  <tr>
    <td align="center"><b>Stationary Balance</b></td>
    <td align="center"><b>Forward / Back</b></td>
    <td align="center"><b>Turning</b></td>
  </tr>
  <tr>
    <td>
      <figure><video src="https://github.com/user-attachments/assets/91283d49-3aaa-4c88-b376-6bd14fcaf858" autoplay loop muted playsinline width="250"></video></figure>
    </td>
    <td>
      <figure><video src="https://github.com/user-attachments/assets/19c978ff-1719-4647-aa42-119d8a422805" autoplay loop muted playsinline width="250"></video></figure>
    </td>
    <td>
      <figure><video src="https://github.com/user-attachments/assets/8aa021a6-f164-49ae-b7fd-ad5f9f7b9121" autoplay loop muted playsinline width="250"></video></figure>
    </td>
  </tr>
</table>
...
- Complementary filter fuses gyroscope and accelerometer: angle = 0.75 x (angle + gyro x dt) + 0.25 x accel_angle
- Tuned gains: Kp = 6.9, Ki = 100, Kd = 0.7 with a static lean offset BASE_OFFSET = 0.9 deg
...
| Pin | Function |
|-----|----------|
| D2, D4 | Right encoder A/B (via 27k/10k voltage divider) |
| D7, D8 | Left encoder A/B (via 27k/10k voltage divider) |
...
![Circuit Schematic](docs/circuit.png)

Arduino Nano 33 BLE Sense drives two independent DRV8871DDA H-bridge channels directly from the 9.6-11 V battery rail through a fuse. Each motor's quadrature encoder outputs are stepped down to 3.3 V via a 27k/10k resistor divider before connecting to the Nano's GPIO.
...
BLE commands dispatch into a state machine with three modes: MOVE, HOLD, and TURN. The balance loop runs at ~100 Hz — a complementary filter produces the pitch angle, which feeds a PID controller whose output drives both motors through deadband compensation. Encoders (ISR on rising edge, 480 counts/rev) feed back into position and heading estimates for distance control and yaw correction.
...
├── firmware/
│   └── self_balancing_robot_final.ino
├── docs/
│   ├── circuit.png
│   └── flowchart.png
└── README.md
...
The robot is controlled via a Flutter iOS app using a custom GATT service. The app was built on top of [Mohamadol/Flutter_Arduino_Bluetooth](https://github.com/Mohamadol/Flutter_Arduino_Bluetooth), adapted to match the robot service/characteristic UUIDs and command set (FORWARD, BACKWARD, LEFT, RIGHT, STOP).
...
| BASE_OFFSET | 0.9 deg | Static CoM lean correction |
